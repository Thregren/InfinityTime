...
- 响应式图片网格 + 懒加载；
- 灯箱大图（单一实例避免重复 DOM；**渐进加载 blur-up：缩略图模糊预览→全图渐入**；上一张/下一张、键盘、滚轮、移动端左右滑动切图；各宽度保留圆角）；
- 主图外侧 **EXIF 侧栏**：标题、描述、拍摄参数、地址，随图片加载/切换自动显示与隐藏；
- 后台上传与图集管理改为 **AJAX**（发布 / 删除图集不整页刷新），选图预览 / 删除 / 逐图标题描述与文件顺序严格对齐；
- 后台**头像 / 站点图标支持直接上传**（自动转 WebP、最长边 ≤512px、质量 78、自动清理旧图）；
- 卡片显示**拍摄时间**（中文日期）；
