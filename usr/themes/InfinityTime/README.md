...
- 缩略图按真实宽高 `aspect-ratio` 预占位，图片加载时**顺序稳定不跳变**（无限加载的新卡片同样生效）；
- 灯箱大图：poptrox 单实例、PC / 移动端统一底部上一张/下一张按钮、**渐进加载 blur-up**；多图图集内切换；
- 灯箱/EXIF 侧栏/主图 blur-up 采用**柔和非线性动效**（`easeInOutQuad` / `cubic-bezier(0.45,0,0.55,1)`）；
...
- 灯箱「拍摄参数」下方**「主题色」**：提取 3 个代表性颜色（色块 + 大写色号）；
- 灯箱**RGB 直方图**：与主题色同一采样，实时显示曝光/色彩分布；
